...
# -*- coding: utf-8 -*-
"""
FCRA Tradeline Violations — Full Diagnostics (Minor+Moderate+Serious+Severe+Extreme)
...
- Moderate: No update in >12 months; near-obsolete DoFD (6y-7.5y); small numeric mismatches across bureaus.
- Minor: Missing optional payment/activity dates; high credit > credit limit; month-only dates; cosmetic balance differences.

Usage:
    python fcra_detect_tradeline_violations_full_updated.py input.csv \
        --out violations_all_severities.csv \
...
# Tolerance constants for cross-bureau checks
SMALL_NUM_TOL = 10  # Moderate cross-bureau tolerance
COSMETIC_TOL = 3    # Minor cross-bureau tolerance
...
class Violation:
    rule_id: str
    rule_name: str
    severity: str  # "Minor", "Moderate", "Serious", "Severe", "Extreme"
    statutes: List[str]
    explanation: str
    creditor_code: str
...
def evaluate_row_rules(row: Dict[str, str], idx: int, discharge_date: dt.date) -> Tuple[List[Violation], List[AuditNote]]:
    ...
    high_credit = parse_money(row.get("high_credit"))
    credit_limit = parse_money(row.get("credit_limit"))
    ...
    # MODERATE — No update in >12 months
    if last_upd and report_date:
        months_since_update = (report_date - last_upd).days / 30.44  # Average days per month
        if months_since_update > 12:
            add("MOD-001", "No Update in >12 Months", "Moderate",
                ["§1681e(b)"], "Account has not been updated in over 12 months, potentially stale data.",
                {"date_reported_last_updated": row.get("date_reported_last_updated"),
                 "months_since_update": round(months_since_update, 1)})

    # MODERATE — Near-Obsolete DoFD (6y-7.5y)
    if dofd and report_date:
        days_since_dofd = (report_date - dofd).days
        if 6 * 365 <= days_since_dofd <= (7 * 365 + 180):
            add("MOD-002", "Near-Obsolete DoFD (6y-7.5y)", "Moderate",
                ["§1681c"], "Date of First Delinquency is approaching obsolescence (6-7.5 years old).",
                {"date_of_first_delinquency": row.get("date_of_first_delinquency"),
                 "years_since_dofd": round(days_since_dofd / 365, 1)})

    # MINOR — Missing optional payment/activity dates
    if not last_pmt and not last_act:
        add("MIN-001", "Missing Optional Payment/Activity Dates", "Minor",
            ["§1681e(b)"], "Both last payment and last activity dates are missing (optional but helpful fields).",
            {"date_of_last_payment": row.get("date_of_last_payment"),
             "date_of_last_activity": row.get("date_of_last_activity")})

    # MINOR — High Credit > Credit Limit
    if high_credit is not None and credit_limit is not None and high_credit > credit_limit:
        add("MIN-002", "High Credit > Credit Limit", "Minor",
            ["§1681e(b)"], "High credit amount exceeds stated credit limit (inconsistent data).",
            {"high_credit": high_credit, "credit_limit": credit_limit})

    # MINOR — Month-only dates (check original string format)
    date_fields = ["date_of_last_payment", "date_of_last_activity", "date_reported_last_updated"]
    for field in date_fields:
        date_str = _strip(row.get(field))
        if date_str and re.match(r"^\d{4}-\d{2}$|^\d{2}/\d{4}$|^\d{4}/\d{2}$", date_str):
            add("MIN-003", "Month-only Dates", "Minor",
                ["§1681e(b)"], f"Date field '{field}' contains only month/year precision (lacks specific day).",
                {field: date_str})
        ...
        for fld in CRITICAL_FIELDS_FOR_XBUREAU:
            ...
            for _, r in group:
                ...
                if fld in {"balance", "amount_past_due", "charge_off_amount"}:
                    parsed_v = parse_money(v)
                    if parsed_v is not None:
                        numeric_values.append(parsed_v)
                    v = parsed_v
                v = str(v)
                values.add(v)
            nonblank = {v for v in values if v not in {"", "None"}}

            # Check for severe inconsistencies first (material differences)
            if len(nonblank) > 1:
                # For numeric fields, check if differences are within tolerance
                if fld in {"balance", "amount_past_due", "charge_off_amount"} and len(numeric_values) > 1:
                    min_val = min(numeric_values)
                    max_val = max(numeric_values)
                    spread = max_val - min_val

                    # Severe: Material difference (> $10)
                    if spread > SMALL_NUM_TOL:
                        idx0, r0 = group[0]
                        ...
                        out.append(Violation(
                            ...
                            severity="Severe",
                            statutes=["§1681e(b)"],
                            explanation=f"Critical field '{fld}' differs materially across bureaus on the same report_date.",
                            ...
                            row_index=idx0,
                            fields={"field": fld, "bureau_values": bureau_vals, "spread": spread}
                        ))
                    # Moderate: Small numeric mismatch ($3.01 - $10)
                    elif spread > COSMETIC_TOL:
                        idx0, r0 = group[0]
                        bureau_vals = { _strip(r.get("bureau")): r.get(fld) for _, r in group }
                        out.append(Violation(
                            rule_id="MOD-004",
                            rule_name=f"Small Cross-Bureau Numeric Mismatch: {fld}",
                            severity="Moderate",
                            statutes=["§1681e(b)"],
                            explanation=f"Field '{fld}' has small numeric differences across bureaus (${spread:.2f} spread).",
                            creditor_code=_strip(r0.get("creditor_code")),
                            creditor_full_name=_strip(r0.get("creditor_full_name")),
                            bureau=_strip(r0.get("bureau")),
                            report_date=_strip(r0.get("report_date")),
                    ...
                    # Minor: Cosmetic balance difference (≤ $3)
                    elif spread > 0:
                        idx0, r0 = group[0]
                        bureau_vals = { _strip(r.get("bureau")): r.get(fld) for _, r in group }
                        out.append(Violation(
                            rule_id="MIN-005",
                            rule_name=f"Cosmetic Cross-Bureau Balance Difference: {fld}",
                            severity="Minor",
                            statutes=["§1681e(b)"],
                            explanation=f"Field '{fld}' has minor cosmetic differences across bureaus (${spread:.2f} spread).",
                            creditor_code=_strip(r0.get("creditor_code")),
                            creditor_full_name=_strip(r0.get("creditor_full_name")),
                            bureau=_strip(r0.get("bureau")),
                            report_date=_strip(r0.get("report_date")),
                    ...
                    # Non-numeric fields - treat as severe inconsistency
                    idx0, r0 = group[0]
                    bureau_vals = { _strip(r.get("bureau")): r.get(fld) for _, r in group }
                    out.append(Violation(
                        rule_id="SEV-XB-001",
...
def main():
    ap = argparse.ArgumentParser(description="Detect all FCRA violations (Minor+Moderate+Serious+Severe+Extreme).")
    ap.add_argument("csv_in", help="Input CSV path")
    ap.add_argument("--out", default="violations_all_severities.csv", help="Output CSV of violations")
    ...
    print(f"Detected {len(violations)} violations (all severities).")
