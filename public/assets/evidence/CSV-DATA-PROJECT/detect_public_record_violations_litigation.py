...
# -*- coding: utf-8 -*-
"""
detect_public_record_violations_litigation.py
Litigation-hardened detector for PUBLIC RECORDS (Serious/Severe/Extreme only).
Reads: public_records.csv (or a provided CSV path), case_context_public_litigation.yaml, public_record_rules_litigation.yaml
Writes: public_record_violations_litigation.csv
"""
...
def eval_rule(row, rule, ctx):
    ...
    # support: all / any / not_any_contains / any_contains / simple / gt_years_from
    def eval_when(node):
        if "all" in node:
...
def main():
    ap = argparse.ArgumentParser(description="Public Records — Litigation-Hardened Violations")
    ap.add_argument("csv_in", nargs="?", default="public_records.csv")
    ap.add_argument("--context", default="case_context_public_litigation.yaml")
    ap.add_argument("--rules", default="public_record_rules_litigation.yaml")
    ap.add_argument("--out", default="public_record_violations_litigation.csv")
    args = ap.parse_args()
